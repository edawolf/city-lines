...
interface TestData {
  anchorPoint?: { label: string; x: number; y: number };
  cornerTest?: boolean;
...
export class ResponsiveDebugTest extends Container {
  ...
  private createPixiUITests(): void {
    ...
    (responsiveButton as Container & { testData?: TestData }).testData = {
      uiTest: true,
      type: "button",
    };
    (responsiveSlider as Container & { testData?: TestData }).testData = {
      uiTest: true,
      type: "slider",
    };
  ...
  private createTestElement(config: {
    ...
    // Store test data
    if (config.data) {
      (element as Container & { testData?: TestData }).testData = config.data;
    }
  ...
  private positionTestElements(): void {
    this.testElements.forEach((element) => {
      const testData =
        (element as Container & { testData?: TestData }).testData || {};
  ...
  private reinitializeAISystem(): void {
    ...
    for (const element of this.testElements) {
      const testData = (element as Container & { testData?: TestData })
        .testData;
   ...
   */
  private getOptimalRegion(_elementIds: string[]): {
    x: number;
    y: number;
    width: number;
    height: number;
  ...
  private traverseAndCollectElements(
    ...
    container.children.forEach((child, index) => {
      if (child instanceof Container) {
        ...
        const testData = (child as Container & { testData?: TestData })
          .testData;
  ...
  private getElementName(
    ...
    const testData = (element as Container & { testData?: TestData }).testData;
