...
interface ParticleConfig {
  ...
  acceleration: number;
  gravity: number;
  rotationStart: number; // Initial rotation angle in degrees
  rotationSpeed: number;
  rotationVariance: number;
  rotationDirection: "clockwise" | "counter-clockwise" | "random";

  // Lifetime
  emitterStartDelay: number; // Delay before emitter starts spawning (in seconds)
  emitterLifetime: number; // How long the emitter spawns particles (0 = infinite)
  emitterLifetimeVariance: number;
  particleLifetime: number; // How long each particle lives
  particleLifetimeVariance: number;
  loop: boolean; // If true, emitter respawns after death; if false, plays once

  // Emitter
  emitting: boolean;
  burst: boolean; // If true, spawn all particles at once
  spawnRate: number; // particles per second
  maxParticles: number;
  particlesPerWave: number;

  // Spawn Shape (commented out for now)
  // spawnType: 'point' | 'circle' | 'rectangle' | 'ring';
  // spawnRadius: number;
  // spawnWidth: number;
  // spawnHeight: number;
...
interface Spawner {
  ...
  emitterLife: number; // Current emitter life
  emitterMaxLife: number; // When to stop emitting (0 = infinite)
  isEmitterDead: boolean; // Has the emitter finished its lifetime?
  hasBurst: boolean; // Track if burst mode has already fired
  delayTimer: number; // Track time elapsed for start delay
  hasStarted: boolean; // Has the emitter started (delay elapsed)?
}
...
export class ParticleEditorScreenV3 extends Container {
